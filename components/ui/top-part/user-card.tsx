...
export const UserCard = () => {
  ...
  useEffect(() => {
    ...
    const pollGamepad = () => {
      ...
      for (const gp of gamepads) {
        if (!gp) continue
        if (gp.buttons[5]?.pressed) pressed = true
      }
  ...
  return (
    <div className="fixed top-10 right-0 z-[1000]">
      <div
        className={clsx(
          "relative bg-[#f3f4f6] border-gray-500 text-gray-500 border-3 py-5 pr-10 w-84 rounded-l-full border-r-0 flex flex-row gap-2 shadow-xl transition-transform duration-150",
          isPressed && "scale-110"
        )}
      >
